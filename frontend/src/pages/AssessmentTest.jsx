...
function AssessmentTest() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const message = 'Test Submitted Successfully! Score: ' + data.score + '/' + data.total + ' - Percentage: ' + data.percentage.toFixed(2) + '% - Readiness: ' + data.readiness_score.toFixed(2);
      alert(message);
    ...
    <div style={{ padding: '30px', fontFamily: 'Arial', maxWidth: '900px', margin: '0 auto' }}>
      ...
      {questions.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '50px' }}>
          <p style={{ fontSize: '18px', color: '#666' }}>No questions available for this category yet.</p>
          <p style={{ marginTop: '20px', color: '#999' }}>Admin needs to add questions first!</p>
          <button onClick={() => navigate('/assessment-dashboard')} style={{ marginTop: '20px', padding: '12px 30px', backgroundColor: '#667eea', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>Back to Dashboard</button>
        </div>
      ) : (
        <div>

...
function AdminRegister() {
  ...
  return (
    <div style={{ maxWidth: '400px', margin: '50px auto', padding: '30px', border: '1px solid #ddd', borderRadius: '10px' }}>
      <h2 style={{ textAlign: 'center', color: '#ff6347' }}>Admin Registration</h2>
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit" style={{ width: '100%', padding: '10px', backgroundColor: '#ffa500', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>Register</button>
      </form>
      {message && <p style={{ color: message.includes('successful') ? 'green' : 'red', textAlign: 'center' }}>{message}</p>}
      <p style={{ textAlign: 'center', marginTop: '15px' }}>Already have an account? <Link to="/admin/login">Login</Link></p>
